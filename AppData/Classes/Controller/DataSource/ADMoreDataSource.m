...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        ...
        header.delegate = self;
        if (section == 1) {
            header.titleLabel.text = [NSString stringWithFormat:@"URL 方案 (%td)",self.appData.urlSchemes.count];
        } else if (section == 2) {
            header.titleLabel.text = [NSString stringWithFormat:@"查询方案 (%td)",self.appData.queriesSchemes.count];
        } else if (section == 3) {
            ...
            header.titleLabel.text = [NSString stringWithFormat:@"授权 (%td)",self.appData.entitlementsIdentifiers.count];
        }
        return header;
    }
}
